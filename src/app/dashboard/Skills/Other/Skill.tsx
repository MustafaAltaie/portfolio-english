...
const Skill = React.memo(({ skill, setForm, setSkillObj, setOldName, handleDelete }: SkillProps) => {
    ...
    return (
        <div ref={wrapperRef} className='skillCard p-3 flex flex-col gap-2 rounded-sm relative' onClick={() => setSetting(!setting)}>
            ...
            <div className='skillSettingsWrapper flex gap-0.5 absolute w-full h-full top-0 left-0'>
                <div className='w-1/2 flexCenter' onClick={() => {setForm(true); setSkillObj(skill); setOldName(skill.imageLink || '')}}>
                    <PencilIcon className='w-5' />
                </div>
                <div className='w-1/2 flexCenter' onClick={() => handleDelete(skill)}>
